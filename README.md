...
- **Auth & utilisateurs (optionnel)** : definis `TORSEARCH_USERNAME` et `TORSEARCH_PASSWORD` pour exiger un login (desactive si l'une manque). Ce compte devient l'**administrateur** au premier demarrage. L'admin gere ensuite les autres comptes dans Reglages → **Utilisateurs**, avec trois roles : **admin** (tout), **membre** (recherche manuelle + ajout direct), **invite** (parcourir et demander). Mots de passe stockes hashes dans `data/users.json`. Voir `.env.example`.
